...
export function isApiEmbeddingModelExists(embeddingModelName: string) {
  return [...OPENAI_EMBEDDING_MODELS, ...GEMINI_EMBEDDING_MODELS].includes(
    embeddingModelName,
  );
}

export async function isOllamaModelExists(
  ollama: Ollama,
...
export const createEmbeddings = (
  ...
  if (OPENAI_EMBEDDING_MODELS.includes(embeddingModelName)) {
    console.log(
      `Creating embeddings for OpenAI model: ${embeddingModelName}, host: ${keys.openai.endpoint}`,
    );
    ...
    console.log(
      `Creating embeddings for Ollama served model: ${embeddingModelName}`,
    );
...
function initChat(
  family: string,
  modelName: string,
  params: InitChatParams,
  isCustomModel = false,
) {
  console.log(
    `Chat with [${family} ${modelName}]`,
    params.endpoint ? `, Host: ${params.endpoint}` : "",
  );
  ...
  if (
    ...
    (isCustomModel || AZURE_OPENAI_GPT_MODELS.includes(modelName))
  ) {
  ...
  if (
    ...
    (isCustomModel || ANTHROPIC_MODELS.includes(modelName))
  ) {
  ...
  if (
    ...
    (isCustomModel || MOONSHOT_MODELS.includes(modelName))
  ) {
    // Reuse openai's sdk
  ...
  if (
    ...
    (isCustomModel || GEMINI_MODELS.includes(modelName))
  ) {
...
export const createChatModel = (
  ...
  const keys = event.context.keys;
  const [familyValue] =
    Object.entries(MODEL_FAMILIES).find(([key, val]) => val === family) || [];

  if (familyValue) {
    const data =
      keys[familyValue as Exclude<keyof ContextKeys, "ollama" | "custom">];
  ...
  if (customModel && MODEL_FAMILIES.hasOwnProperty(customModel.aiType)) {
    return initChat(
      MODEL_FAMILIES[customModel.aiType as keyof typeof MODEL_FAMILIES],
      modelName,
      customModel,
      true,
    )!;
...
function getProxyEndpoint(endpoint: string, useProxy: boolean) {
  ...
  if (
    useProxy &&
    endpoint &&
    config.public.modelProxyEnabled &&
    config.modelProxyUrl
  ) {

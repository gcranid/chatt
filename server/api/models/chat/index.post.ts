...
export default defineEventHandler(async (event) => {
  const { knowledgebaseId, model, family, messages, stream } =
  ...
  if (knowledgebaseId) {
    ...
    const retriever: BaseRetriever = await createRetriever(
      embeddings,
    ...
    const relevant_docs =
      await retriever.getRelevantDocuments(reformulatedQuery);
    console.log("Relevant documents: ", relevant_docs);

    let rerankedDocuments = relevant_docs;

    if (
      (process.env.COHERE_API_KEY || process.env.COHERE_BASE_URL) &&

...
export default defineEventHandler(async (event) => {
  ...
  const exist =
    (await prisma.knowledgeBase.count({ where: { name: name } })) > 0;
  if (exist) {
  ...
  console.log(
    `Created knowledge base ${name}: ${affected.id} by ${currentUser ? currentUser.name : "anonymous"}`,
  );
  ...
  try {
    ...
    const allUrls = await ingestURLs(
      urls,
      `collection_${affected.id}`,
      affected.embedding!,
      event,

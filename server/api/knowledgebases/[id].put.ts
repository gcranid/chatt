...
} from "~/server/utils/knowledgeBase";

export default defineEventHandler(async (event) => {
  const { knowledgeBaseId, uploadedFiles, urls, name, description, isPublic } =
  ...
  if (uploadedFiles.length > 0 || urls.length > 0) {
    ...
    console.log(
      `Update knowledge base ${knowledgeBase.name} with ID ${knowledgeBase.id}`,
    );

    try {
      await ingestDocument(
        uploadedFiles,
        `collection_${knowledgeBase.id}`,
      ...
      for (const uploadedFile of uploadedFiles) {
        ...
        console.log(
          "Knowledge base file created with ID: ",
          createdKnowledgeBaseFile.id,
        );
      }

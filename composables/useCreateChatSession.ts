...
export function useCreateChatSession() {
  ...
  return async function createChatSession(
    params?: Partial<
      Omit<
        ChatSessionBaseData,
        "attachedMessagesCount" | "createTime" | "updateTime"
      >
    >,
  ) {
    ...
    if (chatModels.value.length === 0) {
      toast.add({
        ...
        description: t("chat.noModelFoundDesc"),
        color: "red",
      });
    } else {
      const availableModels = baseData.models.filter((m) =>
        chatModels.value.some((cm) => cm.value === m),
      );

...
export const getKeysHeader = () => ({
  "x-chat-ollama-keys": encodeURIComponent(JSON.stringify(keysStore.value)),
});
...
export const loadOllamaInstructions = async () => {
  try {
    const { instructions } =
      await $fetchWithAuth<
        Record<string, { id: number; name: string; instruction: string }[]>
      >(`/api/instruction/`);
    return instructions;
  } catch (e) {
...
export async function loadKnowledgeBases() {
  const response = await $fetchWithAuth("/api/knowledgebases/").catch(
    () => null,
  );

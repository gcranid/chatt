...
const embeddings = computed(() => [
  ...new Set(
    data.value?.knowledgeBases?.flatMap((el) => el.embedding || []) || [],
  ),
]);

async function onStartChat(data: KnowledgeBase) {
  const chatSessionInfo = await createChatSession({
    title: data.name,
...
const onDelete = async (row: KnowledgeBase) => {
  confirm(
    t("knowledgeBases.deleteConfirm", [
      `<b class="text-primary">${row.name}</b>`,
    ]),
    {
...
<template>
  <div class="max-w-6xl mx-auto">
    <div class="flex items-center mb-4">
      ...
      <UButton icon="i-material-symbols-add" @click="onShowCreate">
        {{ t("global.create") }}
      </UButton>
    </div>
    <ClientOnly>
      <UTable
        ...
        :empty-state="{
          ...
          label: t('global.noData'),
        }"
      >
        <template #name-data="{ row }">
          <ULink
            class="text-blue-600 dark:text-blue-300 hover:text-blue-700 dark:hover:text-blue-200 underline text-wrap text-left"
          ...
          <div class="inline-flex">
            <UPopover mode="hover" :popper="{ placement: 'right' }">
              <UButton
                color="primary"
                variant="soft"
                :label="'' + row.files.length"
              />
              <template #panel>
                <ul class="p-2 list-inside">
                  <li
                    v-for="(el, i) in row.files"
                    :key="el.id"
                    class="list-disc list-inside my-1 p-1"
                  >
                    {{ el.url }}
                  </li>
                </ul>
        ...
        <template #is_public-data="{ row }">
          <div class="">
            <UIcon
              name="i-heroicons-check-16-solid text-primary"
              v-if="row.is_public"
            />
          </div>
        </template>

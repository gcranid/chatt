...
onMounted(async () => {
  ...
  if (sessionList.value.length > 0) {
    if (
      ...
      !sessionList.value.some((el) => el.id === currentSessionId.value)
    ) {
...
async function onTopChat(item: ChatSession, direction: string) {
  ...
  clientDB.chatSessions.update(item.id!, {
    isTop: direction == "up" ? Date.now() : 0,
  });
  sessionList.value = await getSessionList();
}
...
function onDeleteChat(data: ChatSession) {
  confirm(
    t("chat.deleteChatConfirm", [
      data.title || `${t("chat.newChat")} ${data.id}`,
    ]),
    {
...
async function getSessionList() {
  ...
  const result = await clientDB.chatSessions
    .orderBy("updateTime")
    .reverse()
    .toArray();
...
async function updateSessionInfo(
  data: Partial<
    Omit<ChatSession, "id" | "createTime"> & { forceUpdateTitle: boolean }
  >,
) {
  const currentSession = sessionList.value.find(
    (el) => el.id === currentSessionId.value,
  )!;
...
<template>
  <Component
    :is="isMobile ? USlideover : 'div'"
    :class="
      isMobile
        ? 'w-[80vw] max-w-[400px] h-full'
        : 'border-r dark:border-gray-800'
    "
    class="h-full box-border"
  >
    <div class="p-3 border-b border-primary-400/30 flex items-center">
      <h3 class="text-primary-600 dark:text-primary-300 mr-auto">
        {{ t("chat.allChats") }} ({{ sessionList.length }})
      </h3>
      ...
      <UButton
        ...
        @click="emits('closePanel')"
      ></UButton>
    </div>
    <TransitionGroup
      tag="div"
      name="list"
      class="h-[calc(100%-57px)] overflow-auto"
    >
      <div
        ...
        :class="[
          item.isTop
            ? 'bg-primary-300/10 dark:bg-primary-800/10'
            : 'hover:bg-gray-50 dark:hover:bg-gray-700/30',
          currentSessionId === item.id
            ? 'border-l-pink-700/80 dark:border-l-pink-400/80'
      ...
      >
        <div class="w-full flex items-center text-sm h-[32px]">
          <div
            ...
            :class="
              currentSessionId === item.id
                ? 'text-pink-700  dark:text-pink-400 font-bold'
                : 'opacity-80'
            "
          >
            {{ item.title || `${t("chat.newChat")} ${item.id}` }}
          </div>
          <ChatSessionListActionMore
            :data="item"
            class="action-more"

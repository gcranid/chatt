...
const aiTypes = Object.entries(CONFIG_MODELS.MODEL_FAMILIES)
  .filter(([key]) => key !== "moonshot")
  .map(([value, label]) => ({ value, label }));
const defaultModelsMap: Record<
  ContextKeys["custom"][number]["aiType"],
  string[]
> = {
...
const state = reactive(
  Object.assign({}, defaultState, props.value, {
    aiType: defaultAiType,
    models:
      props.value.models.length === 0
        ? defaultModelsMap[defaultAiType]
        : props.value.models,
  }),
);
const modelName = ref("");
...
<template>
  <UForm :state="state" :schema="schema" @submit="onSubmit">
    ...
    <UFormGroup :label="t('settings.endpoint')" class="mb-4" name="endpoint">
      <UInput
        v-model.trim="state.endpoint"
        size="lg"
        :placeholder="t('global.required')"
      />
    </UFormGroup>
    <UFormGroup :label="t('settings.apiKey')" class="mb-4" name="key">
      <UInput
        v-model.trim="state.key"
        size="lg"
        type="password"
        :placeholder="t('global.required')"
      />
    ...
    <UFormGroup :label="t('settings.modelNameSetting')" name="models">
      <div
        class="border border-gray-300 dark:border-gray-700 mt-2 rounded max-w-[400px]"
      >
        <div class="flex items-center px-4 py-2">
          <UInput
            ...
            autocomplete="off"
            class="flex-1"
            @keydown.enter.prevent="onAddModel"
            :placeholder="t('settings.modelNameTip')"
          />
          <UButton
            :disabled="!modelName.trim()"
            class="ml-2"
            @click="onAddModel"
            >{{ t("global.add") }}</UButton
          >
        </div>
        <div
          v-for="(item, i) in state.models"
          :key="item"

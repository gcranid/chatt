  ...
  return [
    {
      ...
      fields: [
        ...
        },
        {
          ...
          value: "ollama.username",
          type: "input",
          placeholder: t("global.optional"),
        },
        {
          label: t("global.password"),
          value: "ollama.password",
          type: "password",
          placeholder: t("global.optional"),
        },
      ],
    },
    {
      ...
      fields: [
        ...
        },
        {
          ...
          value: "openai.proxy",
          type: "checkbox",
          placeholder: t("settings.proxyTips"),
        },
      ],
    },
    {
      ...
      fields: [
        ...
        {
          label: t("settings.azureDeploymentName"),
          value: "azureOpenai.deploymentName",
          type: "input",
        },
        {
          label: t("settings.proxy"),
          value: "azureOpenai.proxy",
          type: "checkbox",
          placeholder: t("settings.proxyTips"),
        },
      ],
...
const currentLLMFields = computed(
  () => LLMList.value.find((el) => el.key === currentLLM.value)?.fields || [],
);
const state = reactive(getData());
const currentCustomServer = computed(() =>
  state.custom.find((el) => el.name === currentLLM.value),
);
  ...
  modal.open(CreateCustomServer, {
    ...
    onCreate: (name) => {
      if (state.custom.some((el) => el.name === name)) {
        toast.add({
          title: t(`settings.customServiceNameExists`),
          color: "red",
        });
        return;
      }
      ...
      keysStore.value = Object.assign(keysStore.value, {
        custom: (keysStore.value.custom || []).concat(data),
      });
...
function onUpdateCustomServer(data: ContextKeys["custom"][number]) {
  const index = state.custom.findIndex(
    (el) => el.name === currentCustomServer.value!.name,
  );
...
function getData() {
  const data = LLMList.value.reduce(
    (acc, cur) => {
      cur.fields.forEach((el) => {
        (acc as any)[el.value] = el.value
          .split(".")
          .reduce((a, c) => (a as any)[c], keysStore.value);
      });
      return acc;
    },
...
function recursiveObject(
  ...
  cb: (keyPaths: string[], value: any) => any,
) {
  const newObj = {} as any;

  function recursive(
    oldObj: Record<string, any>,
    objPart: Record<string, any>,
    keyPaths: string[] = [],
  ) {
  ...
  <ClientOnly>
    <div class="max-w-6xl mx-auto">
      <SettingsCard>
        ...
        <div>
          <UForm
            v-if="currentLLMFields.length > 0"
            :validate="validate"
            :state="state"
            @submit="onSubmit"
          >
              ...
              >
                ...
                <template v-else-if="item.type === 'checkbox'">
                  <label class="flex items-center">
                    <UCheckbox
                      v-model="state[item.value] as boolean"
                    ></UCheckbox>

...
const limitHistorySize = computed(() =>
  Math.max(sessionInfo.value?.attachedMessagesCount || 0, 20),
);
...
const onSend = async (data: ChatBoxFormData) => {
  ...
  const instructionMessage: Array<Pick<ChatMessage, "role" | "content">> =
  ...
  models.value.forEach((m) => {
    ...
    if (model) {
      ...
      const chatMessages = [
        ...
        messages.value
          .filter((m) => {
            if (m.type === "error" || m.type === "canceled") return false;
            return (
              m.role === "user" ||
              (m.role === "assistant" &&
                (m.model === model.value ||
                  model.value ===
                    `${(m as any).family}/${m.model}` /* incompatible old data */))
            );
          })
  ...
  const index =
    "id" in data
      ? messages.value.findIndex(
          (el) => el.id === data.uid || el.id === data.id,
        )
      : -1;
  if (index > -1) {
  ...
  <div class="flex flex-col box-border dark:text-gray-300 md:-mx-4">
    ...
    >
      ...
      <div class="mx-auto px-4 text-center">
        <h2 class="line-clamp-1">
          {{ sessionInfo?.title || t("chat.untitled") }}
        </h2>
    ...
    >
      <ChatMessageItem
        ...
        :show-toggle-button="models.length > 1"
        class="my-2"
        @resend="onResend"
        @remove="onRemove"
      />
    ...
    >
      ...
      >
        <div class="text-muted flex">
          ...
          <UTooltip
            :text="t('chat.attachedMessagesCount')"

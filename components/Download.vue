  ...
  <UForm :state="state" @submit="onDownload">
    <div class="flex flex-col md:flex-row items-center">
      <div class="flex grow w-full gap-2 md:max-w-lg">
        <UInput
          ...
          :placeholder="t('models.modelNameInputPlaceholder')"
          required
        />
      ...
      <div
        class="text-sm text-gray-500 mt-4 md:mt-0 mx-2 shrink-0"
        v-html="t('models.discover')"
      ></div>
    </div>
    <ul
    ...
    >
      <li v-for="(progress, index) in progresses" :key="index">
        ...
        <span class="font-light text-xs text-gray-700" v-else>{{
          progress.status
        }}</span>
      </li>
    </ul>

...
const embeddingList = computed(() => {
  ...
  const getEmbeddingFromKnowledgeBaseList = props.embeddings.filter(
    (e) =>
      ![...OPENAI_EMBEDDING_MODELS, ...GEMINI_EMBEDDING_MODELS].includes(e),
  );
  const ollamaEmbeddingList = [
    ...new Set([
      ...ollamaEmbeddingModels.value.map((e) => e.value),
      ...getEmbeddingFromKnowledgeBaseList,
    ]),
  ];
  return [
    ...
    const arr = items.filter(
      (el) => "slot" in el || el.value.toLowerCase().includes(val),
    );
...
function validate(data: typeof state) {
  ...
  if (!data.embedding)
    errors.push({ path: "embedding", message: t("global.required") });
  return errors;
}
...
async function submit(formData: FormData) {
  ...
  try {
    await $fetchWithAuth(
      isModify.value
        ? `/api/knowledgebases/${props.data!.id}`
        : `/api/knowledgebases/`,
      {
  ...
  <UModal prevent-close>
    <UCard>
      ...
      >
        ...
        <UFormGroup
          :label="t('knowledgeBases.embedding')"
          name="embedding"
          :required="!isModify"
          class="mb-4"
        >
          <div class="flex">
            <UDropdown
              v-model:open="showEmbeddingDropdown"
              :items="embeddingList"
              :ui="{
            ...
            <UInput
              ...
              class="grow"
              autocomplete="off"
              :placeholder="t('knowledgeBases.embeddingInputPlaceholder')"
        ...
        <UFormGroup
          :label="t('knowledgeBases.description')"
          name="description"
          class="mb-4"
        >
        ...
        <UFormGroup
          :label="t('knowledgeBases.publicAccessible')"
          name="public"
          class="mb-4"
        >
          <p class="text-xs mb-2 text-pink-600 dark:text-pink-300">
            {{ t("knowledgeBases.publicAccessibleTip") }}
          </p>
        ...
        <UTabs :items="tabs" class="pt-4">
          <template #files>
            <div class="pt-2 min-h-[200px]">
              <UFormGroup
                :label="t('knowledgeBases.filesAsKB')"
                name="file"
                class="mb-4"
              >
          ...
          <template #urls>
            <div class="pt-2">
              <UFormGroup
                :label="t('knowledgeBases.urlsAsKB')"
                name="urls"
                class="mb-4"
              >
                <UTextarea
                  ...
                  autoresize
                  :maxrows="6"
                  :placeholder="t('global.onePerLine')"
                />
              </UFormGroup>

              <UFormGroup
                :label="t('knowledgeBases.urlPageParser')"
                name="pageParser"
                class="mb-4"
              >
                <USelectMenu
                  v-model="state.pageParser"
                  :options="parserList"
              ...
              <UFormGroup
                :label="t('knowledgeBases.maxDepth')"
                name="maxDepth"
                class="mb-4"
              >
                <div class="flex items-center">
                  <span class="text-primary mr-2 w-6">{{
                    state.maxDepth
                  }}</span>
                  <URange
                ...
                <template #label>
                  <div class="flex items-center">
                    ...
                    <div
                      ...
                      v-html="t('knowledgeBases.useTip')"
                    ></div>
                    <UPopover
                      mode="hover"
                    ...
                    >
                      <div
                        class="i-heroicons-information-circle-20-solid text-lg text-primary"
                      ></div>
                      <template #panel>
                        <div class="p-4">
        ...
        <div class="flex justify-end">
          <UButton color="gray" class="mr-2" @click="onClose()">{{
            t("global.cancel")
          }}</UButton>

...
const instructionContent = computed(() => {
  return (
    instructions.find((el) => el.id === state.instructionId)?.instruction || ""
  );
});

onMounted(() => {
  clientDB.chatSessions.get(props.sessionId).then((res) => {
    if (!res) return;
    Object.assign(
      state,
      pick(res, Object.keys(state) as (keyof typeof state)[]),
    );
  });
});
...
function onClearHistory() {
  props.onClose();
  setTimeout(() => {
    confirm(t("chat.clearConfirmTip"))
      .then(async () => {
...
async function onSave() {
  const knowledgeBaseInfo = knowledgeBases.find(
    (el) => el.id === state.knowledgeBaseId,
  );
  const instructionInfo = instructions.find(
    (el) => el.id === state.instructionId,
  );
  ...
  <UModal prevent-close>
    <UForm :state="state" @submit="onSave">
      <UCard>
        ...
        <UFormGroup
          :label="t('chat.attachedMessagesCount')"
          name="instructionId"
        >
          <div class="flex items-center">
            <span class="mr-2 w-6 text-primary-500">{{
              state.attachedMessagesCount
            }}</span>
            <URange
        ...
        <div class="text-center mt-6">
          <UButton
            icon="i-material-symbols-delete-history"
            color="red"
            @click="onClearHistory"
            >{{ t("chat.clearBtn") }}</UButton
          >
        </div>
        <template #footer>
          <div class="text-right">
            <UButton color="gray" class="mr-2" @click="onReset">{{

...
const tip = computed(() => {
  const s =
    sendModeList.value[0].find((el) => el.value === submitMode.value)?.label ||
    "";
  return ` (${s})`;
});
...
const sendModeList = computed(() => {
  return [
    [
      ...
      {
        label: t("chat.shiftEnter"),
        value: "shift-enter" as const,
        click: onChangeMode,
      },
    ],
  ];
...
const btnTip = computed(() =>
  props.loading ? t("chat.stop") : isMobile.value ? "" : tip.value,
);
...
<template>
  <div
    ...
    :class="[
      isFocus
        ? 'shadow-lg shadow-primary-400/30 dark:shadow-primary-700/20'
        : '',
      { 'border-primary-400 dark:border-primary-700': isFocus },
    ]"
  >
        ...
        <div class="flex items-center ml-auto">
          <ClientOnly>
            <UButton
              ...
              :class="{ 'send-btn': !isMobile }"
              :icon="loading ? 'i-iconoir-square' : 'i-iconoir-send-diagonal'"
              @click="onStop"
            >

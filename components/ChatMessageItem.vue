...
const timeUsed = computed(() => {
  const endTime =
    props.message.type === "loading" ? Date.now() : props.message.endTime;
  ...
  >
    ...
    <div
      ...
      :class="{
        ...
        'flex-row-reverse': !isModelMessage,
      }"
    >
      <div
        class="flex border border-primary/20 rounded-lg overflow-hidden box-border"
        :class="contentClass"
      >
        ...
        <template v-else-if="isModelMessage">
          <div class="p-3 overflow-hidden">
            ...
            <Sources
              v-show="opened"
              :relevant_documents="message?.relevantDocs || []"
            />
          </div>
        ...
        <pre
          ...
          class="p-3 whitespace-break-spaces"
        />
      </div>
